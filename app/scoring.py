...
def refresh_recommendation_scores(db, model_bundle) -> int:
    ...
    Coûteux (transform + predict_proba sur tout le catalogue) — à appeler
    seulement après un entraînement, jamais par requête utilisateur.
    """
    import pandas as pd
    ...
    # Mise à jour en masse via SQLAlchemy Core (bulk update), bien plus rapide
    # que charger chaque objet ORM et faire un setattr un par un sur 100k lignes.
